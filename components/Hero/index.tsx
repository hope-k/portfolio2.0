...
const Hero = ({ pageInfo }: Props) => {
    ...
    const links = [
        {
            name: 'home', svg: (
                <svg className=' stroke-[#ccc] ' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height='1.2em' width='1.2em'>
        ...
        {
            name: 'projects', svg: (
                <svg className='hover:stroke-white stroke-[#ccc] w-full h-full' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height='1.2em' width='1.2em' >
    ...
    return (
        <motion.section ref={ref} layout='size' id='home' className='px-10 snap-center h-screen flex flex-col space-y-8 items-center justify-center text-center overflow-hidden relative  '>
            ...
            <motion.div
                ...
                exit='exit'
                layout='position'
                className={`p-[3px] fixed  ${selectedTab == 0 ? 'top-0' : selectedTab == 2 ? 'md:top-16 top-0' : selectedTab == 4  ? 'bottom-10' : selectedTab == 3 ? '-top-6' :'md:top-4 top-12'} z-[100] backdrop-blur-[15px] bg-[rgba(38,37,37,0.3)] flex flex-row border-[0.0001px] border-[#cccccc42] rounded-md`}
            >
                {
                    links.map((link, i) => (
                        <Link
